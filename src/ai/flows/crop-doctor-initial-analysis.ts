...
const InorganicSolutionSchema = z.object({
  name: z.string().describe('The name of the recommended inorganic solution/action.'),
  productSuggestion: z
    .string()
    .describe(
      'A common product name or chemical type to search for on an e-commerce site, e.g., "Mancozeb" or "Sulfur Fungicide".'
    ),
});
...
const CropDoctorInitialAnalysisOutputSchema = z.object({
  ...
  inorganicSolutions: z.array(InorganicSolutionSchema).describe('A list of inorganic solutions with product suggestions.'),
...
const prompt = ai.definePrompt({
  ...
  5.  Suggest 2-3 simple inorganic (chemical) solutions. For each, provide a 'name' for the action and a 'productSuggestion' which is a searchable term for an e-commerce site.
  ...
  - inorganicSolutions: [{name: "Apply sulfur-based fungicide.", productSuggestion: "Sulfur Fungicide"}, {name: "Use potassium bicarbonate spray.", productSuggestion: "Potassium Bicarbonate"}]
  ...
  - inorganicSolutions: [{name: "No chemical action needed now.", productSuggestion: "None"}, {name: "Monitor for pests preventatively.", productSuggestion: "Pest Trap"}]
