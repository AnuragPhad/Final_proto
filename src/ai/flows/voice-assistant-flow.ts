...
export async function voiceAssistant(input: z.infer<typeof VoiceAssistantInputSchema>): Promise<string> {
  const prompt = `You are Kisan AI, a helpful farming assistant.
    ...
    if (llmResponse.isToolRequest()) {
        ...
        if (toolRequest.name === 'getWeather') {
            toolOutput = await getWeather(toolRequest.input);
        } else if (toolRequest.name === 'getMandiRatesForCommodity') {
            toolOutput = await getMandiRatesForCommodity(toolRequest.input);
        } else {
        ...
        const finalResponse = await run('summarize-tool-output', async () => {
            return ai.generate({
                prompt: `Summarize the following tool output in a user-friendly way. The user's original query was: "${input.query}". The response must be in ${input.language}. Tool Output: ${JSON.stringify(toolOutput)}`,
                model: 'googleai/gemini-2.0-flash',
            });
        });
